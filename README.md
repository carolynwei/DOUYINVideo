...
基于 Streamlit + DeepSeek + 智谱AI + 火山引擎的全自动短视频生成工具。支持工作流模式和对话创作模式两种交互方式。
...
- 🎤 **多音色TTS**：支持火山引擎（京腔侃爷、俊朗男友、甜心小妹）和 Edge-TTS
- 🎬 **视频合成**：自动生成竖屏短视频（1080x1920）
- 🎵 **BGM混音**：自动添加背景音乐
...
├── chat_page.py          # 对话创作页面模块
├── api_services.py       # AI API 调用服务
├── video_engine.py       # 视频渲染引擎
...
3. 选择配音音色
4. 点击"刷新抖音热点"获取热搜
5. 选择目标话题，点击"AI导演"或"爆款大师"生成剧本
6. 在编辑器中微调文案和画面描述
7. 可选：点击"精修剧本"让AI优化
8. 点击"生成大片"开始渲染（2-3分钟）
...
1. **字体文件**: `font.ttf` 必须存在，否则字幕渲染会失败
2. **ImageMagick**: Windows用户需要手动安装
3. **积分系统**: 每次API调用会消耗积分，请注意余额
4. **火山引擎TTS**: 未配置时自动回退到 Edge-TTS
5. **数据库**: `app_data.db` 会自动创建，不要删除
