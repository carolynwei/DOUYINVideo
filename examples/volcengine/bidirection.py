...
async def main():
    ...
    try:
        ...
        all_audio_data = bytearray()  # 合并所有音频数据
        ...
        for i, sentence in enumerate(sentences):
            ...
            if audio_data:
                all_audio_data.extend(audio_data)  # 合并到总数据
                if not args.output:  # 如果没有指定 --output，保留原有逻辑
        ...
        # 如果指定了 --output，将所有音频合并保存到一个文件
        if args.output and all_audio_data:
            with open(args.output, "wb") as f:
                f.write(all_audio_data)
            logger.info(f"All audio merged and saved to {args.output}, total size: {len(all_audio_data)} bytes")
